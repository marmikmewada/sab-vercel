...
const updateMemberProfile = async (req, res) => {
  ...
  console.log(updates, "member console log from server");
  try {
...
const updateHouseholdProfile = async (req, res) => {
  ...
  try {
    ...
    // Define the updates
    const updates = req.body;
    console.log(updates, "household console log from server")
...
const getUserProfile = async (req, res) => {
  try {
    ...
    switch (membership.status) {
      case "applied":
        // If membership status is "applied", return the user profile only
        const userApplied = await User.findById(userId);
        return res.status(200).json({ user: userApplied });
      ...
      case "expired":
        ...
        if (!member) {
          member = new Member({ user: userId });
          await member.save();
        } else {
          // Refresh member data in case it was just created or updated
          member = await Member.findById(member._id);
        }

        // Find or create household associated with the member
        let household = await Household.findOne({ member: member._id });

        if (!household) {
          household = new Household({ member: member._id });
          await household.save();
        } else {
          // Refresh household data in case it was just created or updated
          household = await Household.findById(household._id);
        }
        console.log("user", userActive);
        console.log("member", member);
        console.log("household", household);
        return res.status(200).json({ user: userActive, member, household });
